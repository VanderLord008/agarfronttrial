...
const draw = () => {
  //reset the context translate to default
  context.setTransform(1, 0, 0, 1, 0, 0);
  //clearRect clears out the entire canvas so we can draw anew on the next frame
  context.clearRect(0, 0, canvas.width, canvas.height);
  //clamp the screen to the player
  ...
  players.forEach((p) => {
    if (!p.playerData) {
      //player does not exist that means it was absorbed
      return;
    }
    context.beginPath();
